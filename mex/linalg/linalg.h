...
template<typename T> class Matrix : public Data<T>, public AbstractMatrix<T>, public AbstractMatrixB<T> {
   ...
   /// update only positive values
   inline void update_pos(Matrix<T>& Y);
...
/// update only positive values
template <typename T> inline void Matrix<T>::update_pos(Matrix<T>& Y) {
    for (INTM i = 0; i<_n; ++i) {
      for (INTM j =0; j<_m; ++j) {
          if(Y(j, i) > 0)
              _X[i*_m+j] = Y(j, i);
...
/// replace each value to by absolute value
template <typename T> inline void Matrix<T>::to_abs() {
   Vector<T> vec;
   toVect(vec);
   vec.abs_vec();
};
